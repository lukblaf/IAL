...
**/
	...
	tBTNodePtr *helper = RootPtr;

	while (true){ 
		if (*helper == NULL) { // Ak som našiel ukazovateľ na miesto v pamäti, kde je hodnota NULL, tak tam je miesto, kde budeme vkladať nový prvok stromu.
			(*helper) = malloc(sizeof(struct tBTNode)); // Vyhradíme si miesto v pamäti.
			// Vytvorenie nového uzlu
			(*helper)->Cont = Content; // Obsah a zároveň identifikátor
			(*helper)->LPtr = NULL; // Ľavý sused.
			(*helper)->RPtr = NULL;	// Pravý sused.
			return;
		}
		else if (Content < (*helper)->Cont) helper = &(*helper)->LPtr; // Ak je zadaný kľúč menší než v aktuálnom uzle, tak ideme vľavo.
		else if (Content > (*helper)->Cont) helper = &(*helper)->RPtr; // Ak je zadaný kľúč väčší než v aktuálnom uzle, tak ideme vpravo.
		else return; // Ak už uzol existuje - nič nerobíme.
		}	
}
...
**/
	while (ptr != NULL) {
		...
		SPushP(Stack, ptr); // Uložíme ukazateľ na uzol do zásobníku.
		ptr = ptr->LPtr; // Pokračujeme ďalej smerom doľava.
	}
}
...
**/
	tStackP Stack; // Deklarácia zásobníka.
	SInitP(&Stack); // Inicializácia zásobníka.
	Leftmost_Preorder(RootPtr, &Stack); // V ľavej vetve hľadáme najľavejší uzol.

	while (!SEmptyP(&Stack)) { // Uskutočníme triedenie PreOrder(Left, Right, Root), dokým nebude zásobník prázdny.
...
** realizujte jako volání funkce BTWorkOut().
**/
	tStackP Stack;	// Deklarácia zásobníku
	SInitP(&Stack); // Inicializácia zásobníku;
	Leftmost_Inorder(RootPtr, &Stack); 

	while (!SEmptyP(&Stack)) { // Uskutočníme triedenie InOrder(Left, Root, Right), dokým nebude zásobník prázdny.
		RootPtr = STopPopP(&Stack);
		BTWorkOut(RootPtr); // Spracujeme uzol.
...
**/
	...
	while (ptr != NULL) {
		SPushP(StackP, ptr); // Uložíme ukazateľ na uzol do zásobníku.
		SPushB(StackB, true); // Aktuálny uzol sme navštívili. Uložíme do zásobníku hodnotu 'true' pre daný uzol. 
		ptr = ptr->LPtr; // Pokračujeme ďalej smerom doľava.
	}
...
**/
	...
	tStackB StackB;
	tStackP StackP;
	SInitP(&StackP); // Inicializácia zásobníka ukazovateľov
	SInitB(&StackB); // Inicializácia zásobníka pre hodnoty typu bool.
	Leftmost_Postorder(RootPtr, &StackP, &StackB);

		while (!SEmptyP(&StackP)) {
			RootPtr = STopPopP(&StackP);
			left_friend = STopPopB(&StackB);
			if (left_friend) { //Prichádzame z prava, pojdeme doľava.
				SPushB(&StackB, false);
				SPushP(&StackP, RootPtr);
				Leftmost_Postorder(RootPtr->RPtr, &StackP, &StackB);
			}
			else BTWorkOut(RootPtr); // Spracujeme uzol.
		}
}
...
**/
	tStackP Stack;
	SInitP(&Stack); // Inicializácia zásobníka ukazovateľov.

	do {
		if ((*RootPtr) == NULL){ // Zoberieme uzol do zásobníku.
			if (!SEmptyP(&Stack)) {
				(*RootPtr) = STopPopP(&Stack);
			}
		}
		else {
			if ((*RootPtr)->RPtr != NULL) { // Pravého dáme do zásobníku
				SPushP(&Stack, (*RootPtr)->RPtr);
			}
			struct tBTNode *helper = (*RootPtr);
			*RootPtr = (*RootPtr)->LPtr;	// Ideme doľava.
			free(helper); // Zrušíme aktuálny uzol.
		}
