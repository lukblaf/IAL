...
/* ---KOMENTAR KU RIESENIU---
** Prázdnemu zásobníku zodpovedá hodnota -1, ak tam je jedna položka tak 0. 
** Testujem takýmto princípom:  s->top == -1 ? true : false 
*/
    return (s->top == -1);
}
...
/* ---KOMENTAR KU RIESENIU---
** Prázný zasobník zodpovedá hodnote top == -1 ak tam "pushneme" 5 položiek,
** tak nadobúdame hodnôt top == 4 (začíname pushovat od indexu 0,PRÍČINA: deklarácie DT poľa) a STACK_SIZE == 5.
** Z toho dôvodu porovnávame top s veľkosťou zásobníka dekrementovanú o jedna (4 == 4 ?).
** Ak je zásobník plný navrátová hodnota nadobúda hodnoty TRUE, inak navracia FALSE.
*/
...
*/
    ...
    if (stackEmpty(s)) {
        ...
        *c = s->arr[s->top]; // Priradenie referencie na vrchol zásobníku aby sme videli, čo tam je. Iba sa pozrieme - nemažeme.
...
*/
    ...
    s->top--; // Vstupénie do štruktúri stacku a vykonaná dekrementácia na jeho vrchole.
}
...
*/
    ...
    /* Prevedenie posunu vrchu zásobníku na voľnú pozíciu pre push. 
    ** Následne priradenie pridávaného prvku prostredníctvom parametru c.
    */
    s->arr[++(s->top)] = c;	
